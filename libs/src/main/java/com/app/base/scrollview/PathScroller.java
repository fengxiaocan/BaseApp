...
public class PathScroller {
     ...
     * Start scrolling.
     *
     * @param valueFactor      The "Factor" to scale the "value", @see
     ...
     */
    public boolean computeScrollOffsetY() {
        if (mFinished) {
            return false;
        }
    ...
    public boolean computeScrollOffsetX() {
        if (mFinished) {
            return false;
        }
        final long timePassed = AnimationUtils.currentAnimationTimeMillis() - mStartTime;
        ...
        mCurrValue = Math.round(mValueFactor * mPathPointsHolder.getX(timePassedPercent));
        return true;
    }
    ...
    public static class PathPointsHolder {
         ...
         *
         * @param path A Path that extends from <code>Point</code>
        ...
        float getX(float x) {
            if (x <= 0.0f) {
                return mX[0];// return 0.0f;
            } else if (x >= 1.0f) {
                return mX[mX.length - 1];// return 1.0f;
            }
            ...
            final float xRange = mY[endIndex] - mY[startIndex];
            if (xRange == 0) {
                return mX[startIndex];
            }

            final float tInRange = x - mY[startIndex];
            final float fraction = tInRange / xRange;

            final float startX = mX[startIndex];
            final float endX = mX[endIndex];
